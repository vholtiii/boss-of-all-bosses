    ...
    aiOpponents: allFamilies
      ...
      .map(f => {
        ...
        return {
          ...
          resources: { money: 35000 + Math.floor(Math.random() * 15000), soldiers: Math.max(1, 2 + Math.floor(Math.random() * 2) + (difficulty === 'hard' ? 1 : 0) - (difficulty === 'easy' ? 1 : 0)), influence: 8 + Math.floor(Math.random() * 8), respect: 15 + Math.floor(Math.random() * 10), heat: 0 },
